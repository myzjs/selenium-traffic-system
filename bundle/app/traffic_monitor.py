...
# ---------------- 常量 ----------------
# 26.8.23.1 = 2026-08-23 事件日志失控修复（P0）：
#   ① rt_events.jsonl 无限增长（VPS 实测 3.4GB）：EventStore.add() 每次追加无轮转
#     → 新增 _EVENT_JSONL_MAX_BYTES (100MB) 上限，超限时滚动为 rt_events.jsonl.1（仅保留 1 份备份）
#   ② 同一规则告警无去重冷却，每秒重复写入几十条相同事件
#     → 新增 (rule_id, summary) 维度 60 秒冷却窗口，冷却期内事件只进内存 ring / SSE，不落盘不刷控制台
APP_VERSION = "26.8.23.1"
...
# 26.8.23.1 ★ 事件文件轮转 + 去重冷却 常量
_EVENT_JSONL_MAX_BYTES = 100 * 1024 * 1024      # rt_events.jsonl 单文件硬上限：100MB
_EVENT_JSONL_BACKUP = "rt_events.jsonl.1"        # 滚动后保留 1 份备份（覆盖旧备份）
_EVENT_DEDUP_COOLDOWN_SEC = 60.0                # 同一 (rule_id, summary) 60 秒内只落盘 1 条
...
class EventStore:
    def __init__(self, events_dir: str):
        ...
        # 26.8.23.1 ★ 去重冷却状态：key=(rule_id, summary) → 最近一次落盘时间
        self._dedup_last_ts: Dict[Tuple[str, str], float] = {}
        # 26.8.23.1 ★ 被冷却抑制的事件数（供 /monitoring API 观测）
        self.dedup_suppressed_count: int = 0

    # ----- 26.8.23.1 ★ 文件轮转：rt_events.jsonl 超 100MB 时滚动为 .jsonl.1 -----
    def _rotate_if_needed(self) -> None:
        try:
            if self.jsonl_path.stat().st_size >= _EVENT_JSONL_MAX_BYTES:
                backup = self.dir / _EVENT_JSONL_BACKUP
                if backup.exists():
                    backup.unlink()
                shutil.move(str(self.jsonl_path), str(backup))
                _MON.warning(
                    "📦 rt_events.jsonl 已达 %dMB，滚动为 %s（旧备份已覆盖）",
                    _EVENT_JSONL_MAX_BYTES // (1024 * 1024), _EVENT_JSONL_BACKUP,
                )
        except FileNotFoundError:
            pass  # 首次写入前文件不存在属正常
        except Exception as _rot_e:
            _MON.error("rt_events.jsonl 轮转失败(不阻断写入): %s", _rot_e)

    # ----- 26.8.23.1 ★ 去重冷却：同一 (rule_id, summary) 冷却期内不落盘 -----
    def _is_suppressed_by_cooldown(self, ev: RTEvent) -> bool:
        key = (ev.rule_id, ev.summary)
        now = time.time()
        last = self._dedup_last_ts.get(key)
        if last is not None and (now - last) < _EVENT_DEDUP_COOLDOWN_SEC:
            return True
        self._dedup_last_ts[key] = now
        # 防止 key 无限增长：超过 500 个 key 时清理过期项
        if len(self._dedup_last_ts) > 500:
            cutoff = now - _EVENT_DEDUP_COOLDOWN_SEC
            self._dedup_last_ts = {
                k: t for k, t in self._dedup_last_ts.items() if t > cutoff
            }
        return False
    ...
    def add(self, ev: RTEvent) -> None:
        with self._lock:
            ...
            # 26.8.23.1 ★ 冷却期内：只进内存 ring / SSE 推送，不写文件、不重复刷控制台
            if not self._is_suppressed_by_cooldown(ev):
