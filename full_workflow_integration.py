...
def test_risk_detection(page, proxy_info):
    ...
    try:
        ...
        return {
            ...
            "risk_reasons": calc.get("risk_reason_list", [])
        }
...
def test_browsing_rules(page, config):
    logger.info("\n" + "=" * 60)
    logger.info("测试阶段5：按规则浏览网页")
    logger.info("=" * 60)
    
    try:
        browse_rules = config.get("browse_rules", {})
        
        stay_min = browse_rules.get("stay_min", 15)
        stay_max = browse_rules.get("stay_max", 60)
        stay_time = random.uniform(stay_min, stay_max)
        
        logger.info(f"页面停留时间: {int(stay_time)}秒")
        
        scroll_depth = browse_rules.get("scroll_depth", 0.8)
        logger.info(f"滚动深度: {int(scroll_depth * 100)}%")
        
        click_links = browse_rules.get("click_links", True)
        if click_links:
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            risk_result = test_risk_detection(page, ip_result)
            results["risk_detection"] = risk_result
            
            human_result = simulate_human_behavior(page, CONFIG)
            results["human_behavior"] = human_result
            
            search_result = test_search_engine_navigation(
                page,
                search_query="python selenium tutorial",
                target_domain="w3schools.com"
            )
            results["search_navigation"] = search_result
            
            if search_result.get("success"):
                browse_result = test_browsing_rules(page, CONFIG)
